...
<h2 style="text-align:center; margin-top:20px;">Daftar buku</h2>
...
<table class="table table-bordered ">
 ...
 while($row=mysqli_fetch_assoc($result)) { ?>
    <tr class="table-active">
     <td><?= $no ?></td>
     <td> <img src="img/<?= $row["gambar"];?>" alt="" width="50px" style="margin:auto; display:block; "></td>
     <td><?= $row["nama"]; ?></td>
     <td><?= $row["kode_buku"]; ?></td>
     <td><?= $row["kategori_buku"];?></td>
     <td><?= $row["stok buku"]; ?></td>
     <td><?= $row["tahun_buku"]; ?></td>
     <td><a href="edit.php?id=<?= $row["id_buku"]; ?>"> Edit</a> | <a href="hapus.php?id=<?= $row["id_buku"]; ?>" onclick="return confirm('yakin?')">Hapus</a></td>
